...
// write request header interface for duckDuckGo
...
// function to get duckDuckGo search results from axios
const fetchDuckDuckGoSearchData = async function (searchQuery) {
  // encode search query to represent UTF-8, URLs can only have certain characters from ASCII set
  const encodedSearch = encodeURI(searchQuery);

  // default united states
  const countrySearch = "&kl=us-en";

  // wrap inside OPTIONS function callback to return request headers
  return OPTIONS().then(async function (data) {
    // duckDuckGo search data with axios
    const duckDuckGoSearchData = await axios.get(
      `https://www.duckduckgo.com/?q=${encodedSearch}${countrySearch}`,
      data
    );
    // new duckDuckGo search data promise
    const newDuckDuckGoSearchDataPromise = await duckDuckGoSearchData.data;

    return { promise: newDuckDuckGoSearchDataPromise, requestHeader: data };
  });
};
...
// return array of result objects from duckDuckGo search data
const buildDuckDuckGoResultObject = async function (searchQuery, pLanguage) {
  ...
  // calls fetchDuckDuckGoSearchData from axios (promise)
  const searchData = fetchDuckDuckGoSearchData(searchQuery + " " + encodedPLanguage);
  ...
  return searchData.then(async function (data) {
    ...
    // loop through html class ".nrn-react-div" to article tag to second div, h2, a tag then getting hyperlink
    $(".nrn-react-div > acticle > div:nth-child(2) > h2 > a").each((index, element) => {
      links[index] = $(element).attr("href");
    });

    /// loop through html class ".nrn-react-div" to article tag to second div, h2, a tag then getting text
    $(".nrn-react-div > acticle > div:nth-child(2) > h2 > a > span").each((index, element) => {
      titles[index] = $(element).text();
    });

    // loop through html class ".nrn-react-div" to article tag to third div, div, span, getting text
    $(".nrn-react-div > acticle > div:nth-child(3) > div > span").each((index, element) => {
...
// function to get data from the first link in the duckDuckGo result object from axios
const fetchFirstDuckDuckGoResultPage = async function (searchQuery, pLanguage) {
  // array of result objects, holds the top three results (link, title) from duckDuckGo
  const resultsDuckDuckGo = buildDuckDuckGoResultObject(searchQuery, pLanguage);

  await sleep(1000);
  // data is array of duckDuckGo result objects
  // makes call to axios to get data from the first link of duckDuckGo result object
  return await resultsDuckDuckGo.then(async function (data) {
    await sleep(1000);

    // console.log("duckDuckGo linkstate 1", data.results[0].link);

    // link data from array of duckDuckGo result object with axios
...
// function to get data from the second link in the duckDuckGo result object from axios
const fetchSecondDuckDuckGoResultPage = async function (searchQuery, pLanguage) {
  // array of result objects, holds the top three results (link, title) from duckDuckGo
  const resultsDuckDuckGo = buildDuckDuckGoResultObject(searchQuery, pLanguage);

  await sleep(1000);
  // data is array of duckDuckGo result objects
  // makes call to axios to get data from the second link of duckDuckGo result object
  return await resultsDuckDuckGo.then(async function (data) {
    await sleep(1000);

    //console.log("duckDuckGo linkstate 2", data.results[1].link);
...
// function to get data from the third link in the duckDuckGo result object from axios
const fetchThirdDuckDuckGoResultPage = async function (searchQuery, pLanguage) {
  // array of result objects, holds the top three results (link, title) from duckDuckGo
  const resultsDuckDuckGo = buildDuckDuckGoResultObject(searchQuery, pLanguage);

  await sleep(1000);
  // data is array of duckDuckGo result objects
  // makes call to axios to get data from the third link of duckDuckGo result object
  return await resultsDuckDuckGo.then(async function (data) {
    await sleep(1000);

    //console.log("duckDuckGo linkstate 3", data.results[2].link);
...
// function to get data from the fourth link in the duckDuckGo result object from axios
const fetchFourthduckDuckGoResultPage = async function (searchQuery, pLanguage) {
  // array of result objects, holds the top three results (link, title) from duckDuckGo
  const resultsDuckDuckGo = buildDuckDuckGoResultObject(searchQuery, pLanguage);
  ...
  return await resultsDuckDuckGo.then(async function (data) {
    ...
    //console.log("duckDuckGo linkstate 4", data.results[3].link);
...
// function to get data from the fifth link in the duckDuckGo result object from axios
const fetchFifthDuckDuckGoResultPage = async function (searchQuery, pLanguage) {
  // array of result objects, holds the top three results (link, title) from duckDuckGo
  const resultsDuckDuckGo = buildDuckDuckGoResultObject(searchQuery, pLanguage);
  ...
  return await resultsDuckDuckGo.then(async function (data) {
    ...
    //console.log("duckDuckGo linkstate 5", data.results[4].link);
...
const getResultDataLinks = async function (searchQuery, pLanguage, linkState) {
  ...
  if (linkState === "1") {
    const duckDuckGoPageOne = fetchFirstDuckDuckGoResultPage(searchQuery, pLanguage);

    return duckDuckGoPageOne
    ...
    const duckDuckGoPageTwo = fetchSecondDuckDuckGoResultPage(searchQuery, pLanguage);

    return duckDuckGoPageTwo
      .then(async function (data) {
        // load markup with cheerio
        let $ = cheerio.load(data.linkPromise);
    ...
    const duckDuckGoPageThree = fetchThirdDuckDuckGoResultPage(searchQuery, pLanguage);

    return duckDuckGoPageThree
      .then(async function (data) {
        // load markup with cheerio
        let $ = cheerio.load(data.linkPromise);
    ...
    const duckDuckGoPageFour = fetchFourthDuckDuckGoResultPage(searchQuery, pLanguage);

    return duckDuckGoPageFour
      .then(async function (data) {
        // load markup with cheerio
        let $ = cheerio.load(data.linkPromise);
    ...
    const duckDuckGoPageFive = fetchFifthDuckDuckGoResultPage(searchQuery, pLanguage);

    return duckDuckGoPageFive
      .then(async function (data) {
        // load markup with cheerio
        let $ = cheerio.load(data.linkPromise);
...
module.exports = {
  buildDuckDuckGoResultObject,
  getResultDataLinks,
};

let test = buildDuckDuckGoResultObject("hello world", "c++");
